...
if grep -R "internal/platform/" internal/xdp --include="*.go" >/dev/null 2>&1; then
  echo "internal/xdp imports internal/platform"
  exit 1
else
  echo "internal/xdp platform import guard passed"
fi

xdp_imports=$(go list -f '{{join .Imports "\n"}}' ./internal/xdp)
if echo "$xdp_imports" | grep -E '^github\.com/netxfw/netxfw/cmd/' >/dev/null 2>&1; then
  echo "internal/xdp imports cmd layer"
  exit 1
else
  echo "internal/xdp cmd import guard passed"
fi

go list -deps ./internal/xdp >/dev/null
