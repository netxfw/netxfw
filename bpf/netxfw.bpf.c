...
static inline int check_ip_port_rule(__u32 ip, __u16 port) {
    struct lpm_ip4_port_key key = {
        .prefixlen = 48, // 16 bits port + 32 bits ip
        .port = port,
        .pad = 0,
        .ip = ip,
    };
...
static inline int check_ip6_port_rule(struct in6_addr *ip, __u16 port) {
    struct lpm_ip6_port_key key = {
        .prefixlen = 144, // 16 bits port + 128 bits ip
        .port = port,
        .pad = 0,
    };
...
int xdp_firewall(struct xdp_md *ctx) {
    ...
    if (h_proto == bpf_htons(ETH_P_IP)) {
        ...
        // 3. Check Default Deny and Port Allow List / 检查默认拒绝和端口白名单
        ...
        if (dest_port > 0) {
            // Check IP+Port rules first (most specific) / 先检查 IP+端口规则（最精确）
