...
- **插件索引**: 插件占据 `jmp_table` 的 `2` 到 `14` 号索引位
- **核心逻辑**: 如果索引位有程序，则执行插件逻辑
- **兜底逻辑**: 插槽 1 为主程序，插槽 15 为默认拒绝（安全网）

### 插件返回规范 (重要)

为确保安全防护体系的完整性，插件在判定流量合规后，**必须**将流量交回给 netxfw 的核心逻辑进行后续检查（如连接跟踪、黑白名单等），而**不能**直接返回 `XDP_PASS`。

**正确做法**:
显式尾调用跳转到主程序 (索引 1):
```c
// 包含协议定义以获取 PROG_IDX_MAIN
...
// 如果 tail_call 失败（极其罕见），返回 DROP 以策安全
return XDP_DROP;
```
