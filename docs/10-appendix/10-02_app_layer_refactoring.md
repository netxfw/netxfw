...
> **状态**：部分执行（阶段 2 部分完成，其余阶段待执行）
>
> `ops_*.go` 文件目前仍在 `internal/app/` 目录下，尚未按本计划完全迁移。
> `internal/utils/xdputil/` 已创建并完成实现；`configutil/`、`envutil/` 尚未创建。
...
├── config/            # ✅ 配置服务（职责清晰）
├── plugin/            # ✅ 插件服务（职责清晰）
├── ops_format.go      # ⚠️ 格式化工具（应移至 utils/）
...
| 阶段 | 时间 | 风险 | 状态 |
|------|------|------|------|
| 创建新包 | 第 1 周 | 低 | ✅ 部分完成（xdputil） |
| 实现新函数 | 第 1 周 | 低 | ✅ 部分完成（xdputil） |
