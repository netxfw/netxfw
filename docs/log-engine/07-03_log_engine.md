...
| 动作值 | 字符串形式 | 别名 (Aliases) | 类型 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| `0` | `log` | - | 仅日志 | 仅记录告警日志，不执行拦截。 |
| `1` | `dynblock` | `dynblack`, `dynlock` | 动态封禁 | **默认推荐**。将 IP 加入临时黑名单 (LRU Map)。支持自动过期。 |
| `1` | `dynblock:10m` | `dynblack:..`, `block:..`, `black:..` | 动态封禁 | 封禁指定时长（如 10m, 1h, 30s）。**注意**: 即使前缀是 `block`，只要带时长就是动态。 |
| `2` | `lock` | `deny`, `blacklist`, `block`, `black` | 静态封禁 | **永久封禁**。将 IP 加入持久化黑名单 (LPM Trie)。需手动解封。 |

> **注意**: 
> 1. **数字形式支持**: 动作支持数字形式（`0`/`1`/`2`）或字符串形式，两种写法等效。
> 2. **关键字兼容性**: `block` 和 `black` (无时长) 均视为**静态封禁** (`lock` / `2`)。
> 3. **智能时长识别**: 任何带有 `:duration` 后缀的动作 (如 `block:5m`, `black:1h`) 均视为**动态封禁** (`1`)。
> 4. **推荐用法**: 
>    - 动态: `dynblock` 或 `dynblock:10m` 或 `1`
>    - 静态: `lock` 或 `deny` 或 `2`
>    - 仅日志: `log` 或 `0`
