 ...
 *
 * Phase 3 mapping: pinned-manager bootstrap belongs to
 * internal/datapath/xdp/programs and internal/datapath/xdp/maps.
 */
		...
		for name, pinnedMap := range opts.MapReplacements {
			if specMap, ok := spec.Maps[name]; ok {
				if specMap.MaxEntries != pinnedMap.MaxEntries() {
					// Update spec to match pinned map capacity and avoid map-size mismatch errors
