...
func NewManager(cfg types.CapacityConfig, logger Logger) (*Manager, error) {
	...
	// Note: Using old map names for bpf2go compatibility
	// 注意：使用旧的 Map 名称以兼容 bpf2go
	if cfg.Conntrack > 0 {
...
// Uses bpf2go generated field names (old names for compatibility)
// 使用 bpf2go 生成的字段名称（旧名称以兼容）
func (m *Manager) initMapReferences(objs *NetXfwObjects) {
	// Core maps - using bpf2go generated names
	// 核心 Map - 使用 bpf2go 生成的名称
	m.conntrackMap = objs.ConntrackMap
	m.staticBlacklist = objs.LockList     // bpf2go: lock_list -> staticBlacklist
	m.dynamicBlacklist = objs.DynLockList // bpf2go: dyn_lock_list -> dynamicBlacklist
	m.whitelist = objs.Whitelist
	m.ruleMap = objs.IpPortRules        // bpf2go: ip_port_rules -> ruleMap
	m.topDropMap = objs.DropReasonStats // bpf2go: drop_reason_stats -> topDropMap
	m.topPassMap = objs.PassReasonStats // bpf2go: pass_reason_stats -> topPassMap
	m.globalConfig = objs.GlobalConfig
	m.jmpTable = objs.JmpTable
	m.xskMap = objs.XskMap

	// Stats maps (will be merged into statsGlobalMap in future)
	// 统计 Map（将来会合并到 statsGlobalMap）
	m.statsGlobalMap = nil // Not yet generated by bpf2go

	// Rate limit maps (will be merged into ratelimitMap in future)
	// 速率限制 Map（将来会合并到 ratelimitMap）
	m.ratelimitMap = nil // Not yet generated by bpf2go

	// Critical blacklist (new map, not yet generated)
	// 危机封锁（新 Map，尚未生成）
	m.criticalBlacklist = nil // Not yet generated by bpf2go
}
...
func NewManagerFromPins(path string, logger Logger) (*Manager, error) {
	...
	// Map names (using bpf2go generated names)
	// Map 名称（使用 bpf2go 生成的名称）
	mapNames := []string{
		"conntrack_map",
		"lock_list",
		"dyn_lock_list",
