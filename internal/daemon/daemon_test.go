...
// TestCleanupLoop_Enabled tests cleanup loop when enabled
// TestCleanupLoop_Enabled 测试启用时的清理循环
func TestCleanupLoop_Enabled(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// TestCleanupLoop_ContextCancellation tests cleanup loop stops on context cancellation
// TestCleanupLoop_ContextCancellation 测试上下文取消时清理循环停止
